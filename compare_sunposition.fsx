...
let sunPositionCompute32 (longitudeInDegrees: float32) (latitudeInDegrees: float32) (jdMinusJ2000 : float32) : V3f =
            
    let julianCenturiesPerDay = 1.0f / 36525.0f
    
    // Earth orbital parameters
    let earthPerihelion = 102.9373f // degrees
    let earthMeanAnomalyAtJ2000 = 357.5291f * ConstantF.RadiansPerDegree
    let earthMeanMotion = 0.985608f * ConstantF.RadiansPerDegree // degrees per day
    
    // Earth anomaly approximation coefficients (degrees)
    let earthAnomalyCoeffs = [| 1.9148f; 0.0200f; 0.0003f |]
    
    // 1. Calculate Mean Anomaly
    let meanAnomaly = earthMeanAnomalyAtJ2000 + earthMeanMotion * jdMinusJ2000
    ...
    for c in earthAnomalyCoeffs do
        ...
        mi <- mi + meanAnomaly
    let trueAnomaly = meanAnomaly + equationOfCenter * ConstantF.RadiansPerDegree
    
    // 3. Calculate ecliptic longitude (lambda)
    let lambda = trueAnomaly + (earthPerihelion + 180.0f) * ConstantF.RadiansPerDegree
    
    // 4. Calculate Earth's obliquity (epsilon) using Astronomical Almanac 2010 approximation
    let t = jdMinusJ2000 * julianCenturiesPerDay
    let t2 = t * t
    let t3 = t2 * t
    let t4 = t2 * t2
    let t5 = t3 * t2
    ...
    let epsilon = epsilonArcsec / 3600.0f * ConstantF.RadiansPerDegree
    ...
    let theta = ConstantF.PiTimesTwo * (0.7790572732640f + 1.002737811911354f* jdMinusJ2000)
    
    // 7. Convert to observer coordinates
    let phi = latitudeInDegrees * ConstantF.RadiansPerDegree
    let lw = -longitudeInDegrees * ConstantF.RadiansPerDegree // convert East to West
    ...
    let thetaSphere = ConstantF.PiHalf - height
...
let gpuSunDirection (geo: GeoInfo) =
    ...
    //y = year + 4800L - (14L - 1L) / 12L
    //jd - j2000 = 1L + (153L * m + 2L) / 5L + 365L * y + y / 4L - y / 100L + y / 400L - 32045L - 0.5f + secondsSinceStartOfYear / 86400.0f - j2000
    let totalSecondsSinceStartOfYear = (utcTime - DateTime(utcTime.Year, 1, 1)).TotalSeconds
    let y = utcTime.Year + 4800 - (14 - 1) / 12
    let ji = 1 + (153 * (1 + 12 * ((14 - 1) / 12) - 3) + 2) / 5 + 365 * y + y / 4 - y / 100 + y / 400 - 32045
    let jdMinusJ2000 = float ji - 0.5 + totalSecondsSinceStartOfYear / 86400.0 - 2451545.0
    sunPositionCompute32
        (float32 geo.gpsLong)
        (float32 geo.gpsLat)
        (float32 jdMinusJ2000)
